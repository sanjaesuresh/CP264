...
-------------------------------------
File:    name_list.c
Project: 202301_CP264_Lab05_LinkedLists
file description
-------------------------------------
...
void insert_name_list (sNAME_LIST *name_list, sNODE *node){
   sNODE *current = name_list -> front;
   int found = 0;

   if (name_list == NULL) printf("*name_list is a null list. please set values correctly.");
   ...
   if (current == NULL){
      ...
      while (!found){
         if (strcmp(current -> name.first_name, node -> name.first_name) == 0 &&
            strcmp(current -> name.last_name, node -> name.last_name) == 0){
               found = 1;
         }
