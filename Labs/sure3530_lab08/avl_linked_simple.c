/*QUESTION:::::
 -------------------------------------------------------
 avl_linked_simple.c
 Simple Linked version of the AVL ADT.
 -------------------------------------------------------
 Author:       Sanjae Suresh Kumar
 ID:           210523530
 Email:        sure3530@mylaurier.ca
 Modified Version:      2023-03-14   
 -------------------------------------------------------
 */
...
//==============================================
{
	...
	avl_node *moved_node = child -> left;
	
	child -> left = node; //move nodes for left rotate
	node -> right = moved_node;
	...
	if(child->left == NULL && child->right == NULL){
		...
		child->height = child->left->height +1;
	} else if(child->left != NULL && child->right == NULL){
		child->height = child->right->height +1;
	} else {
		child->height = MAX_HEIGHT(child->left->height, child->right->height)+1;
	}
...
//===============================================
{
	...
	avl_node *moved_node = child -> right;
	// Rearrange the nodes.

	child -> right = node;
	node -> left = moved_node;
...
//================================================
{
	...
	if (node -> left == NULL && node -> right == NULL){
		...
		if (!(node -> height == MAX_HEIGHT(node->left->height, node->right->height) + 1)){
			is_valid *= 0; //reset
		}
	}
	
	//compare using max and min
	if (min_node != NULL && max_node != NULL){ //both null
		if (!(data > min_node -> data && data < max_node -> data)) is_valid *= 0;
	} else if (min_node == NULL && max_node != NULL){
		if (data >= max_node -> data) is_valid *= 0;
	}else if (min_node != NULL && max_node == NULL){
		if (data <= min_node -> data) is_valid *= 0;
	}

	is_valid = is_valid * avl_valid_aux(tree, node->left, min_node, node);
	is_valid = is_valid * avl_valid_aux(tree, node->right, node, max_node);
...
//============================
{
	...
	valid = avl_valid_aux(tree, tree->root, NULL, NULL);
